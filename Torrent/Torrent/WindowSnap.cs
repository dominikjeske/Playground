...
namespace MinimizeCapture
{
    ...
    {
        private string _screenLocation;
        ...
        {
            var torrent = WindowSnap.GetAllWindows(true, true).FirstOrDefault(x => x.Text.StartsWith("µTorrent", StringComparison.InvariantCultureIgnoreCase));
            if (torrent != null)
            {
                torrent.Image.Save(_screenLocation, ImageFormat.Jpeg);
            }
        }

        private void TryReadConfig()
        {
            if (!string.IsNullOrWhiteSpace(_screenLocation)) return;

            var configuration = GetConfiguration();

            _screenLocation = configuration.GetValue<string>("ScreenLocation");
        }
